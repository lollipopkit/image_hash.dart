...
void main() {
  ...
  group('Difference Hash', () {
    ...
    test('horizontal direction', () {
      final exp1 = _differenceHash('origin.png', 'edited.jpg', direction: HashDirection.horizontal);
      expect(exp1, completion(greaterThan(0)));
    });

    test('vertical direction', () {
      final exp1 = _differenceHash('origin.png', 'edited.jpg', direction: HashDirection.vertical);
      expect(exp1, completion(greaterThan(0)));
    });
  });
  ...
  group('ImageHash class', () {
    ...
    test('similarity and isSimilar', () async {
      final img1 = decodeImage(await File('${_testDir}origin.png').readAsBytes());
      final img2 = decodeImage(await File('${_testDir}compressed.jpg').readAsBytes());
      final img3 = decodeImage(await File('${_testDir}transformed.jpg').readAsBytes());
  ...
  group('Batch operations', () {
    test('batchCompareSimilarity', () async {
      final img1 = decodeImage(await File('${_testDir}origin.png').readAsBytes());
      final img2 = decodeImage(await File('${_testDir}compressed.jpg').readAsBytes());
      final img3 = decodeImage(await File('${_testDir}edited.jpg').readAsBytes());
      ...
      expect(similarities[0], greaterThanOrEqualTo(0.9)); // compressed should be very similar
    ...
    test('batchCompareDistance', () async {
      ...
      expect(distances[1], greaterThan(0)); // edited should have non-zero distance
    });
  });
}
